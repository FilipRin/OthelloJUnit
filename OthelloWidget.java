...
//			System.out.println("Testing");
		...
		if (s.isEmpty()) {
			...
			if ((checkWin() == true) && (_winnerName != (null))) {
				...
				if ((checkDraw() == true) && (_winnerName == null)) {
    ...
    public boolean hasLegalMoves (int color, Spot piece)
    {
        ...
        for (int yy = 0; yy < _size; yy++) {
            for (int xx = 0; xx < _size; xx++) {
            	...
            	if(getColorC(xx,yy)==Color.BLACK ) {
            		if( getColor(this._board.getSpotAt(xx, yy))==1)
            		continue;
            	}
                Piece.x = xx;
                Piece.y = yy;
        ...
        for (int ii = 0; ii < DX.length; ii++) {
            ...
            for (int dd = 0; dd < _size; dd++) {
                ...
                if (color == -1) {
                	...
                	toflip.add(this._board.getSpotAt(x, y));
